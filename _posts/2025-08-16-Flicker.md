---
layout: post
title:  "Flicker"
summary: "Solo Developer"
date:   2025-08-16 15:39:40
preview: /assets/flicker_preview.png
---

![Title Image](/assets/flicker_banner.png)

**Overview:**
Submission for the 7 day 2025 Epic Megajam. Entered for the 'One Man Army' and 'Use It or Lose It' special modifiers.

Developed for the theme "However vast the darkness, we must supply our own light"

Some audio assets come from weloveindies, all other game assets made during jam.

**Notable Features**

- Only a single 512x512 RGBA texture is sampled during the game, using a combination of gradient texturing and proceedural or noise based post-processing effects (sampled from the texture) for most shaders.

- Animation for character baked from new procedural locomotor plugin introduced in UE 5.6 (excluding additive layers).

- Fire effects are created using a blend based upon sine of a signed distance field multiplied with the time since burn activation.

**Platform and Engine:**  
   Unreal Engine 5 for PCVR platforms

See the [itch.io page](https://itch.io/jam/2025-epic-megajam/rate/3985332 "flicker itch page") to play.

![Flicker screenshot 1](/assets/flicker1.png)

![Flicker screenshot 2](/assets/flicker2.png)

![Flicker screenshot 3](/assets/flicker3.png)

![Flicker screenshot 4](/assets/flicker4.png)

Link to [gameplay footage](https://youtu.be/ZTP6DVDzDF4 "flicker gameplay").
   